...
export const FacilitatorLoginPage: React.FC<FacilitatorLoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleDemoPreview = () => {
    const mockToken = "mock-facilitator-token";
    localStorage.setItem("ip_sakti_facilitator_token", mockToken);
    setAuth(
      {
        id: "fac-preview-001",
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">
              Ministry of Ayush • Safety & Reliability Human Escalation Workspace
            </p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-emerald-500/25 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-emerald-950/20 dark:shadow-emerald-950/40 space-y-5 text-slate-900 dark:text-white transition-colors duration-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              ...
              <span>Explore as Verified IP Facilitator (Instant Preview)</span>
            </button>
          </form>
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 text-[11px] text-slate-600 space-y-1.5">
            ...
            <p>Email: <code className="text-emerald-900 bg-emerald-100/80 px-1.5 py-0.5 rounded font-mono font-medium">facilitator@ayush.gov.in</code></p>
            <p>Password: <code className="text-emerald-900 bg-emerald-100/80 px-1.5 py-0.5 rounded font-mono font-medium">Facilitator@123</code></p>
          </div>
        </div>

        <p className="text-center text-[11px] text-slate-500">
          The human facilitator desk acts as an authoritative safety fallback for the AI engine.
        </p>
      </div>
    </div>
  );
